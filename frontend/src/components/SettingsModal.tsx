...
import { X, Key, Sliders, Server } from "lucide-react";
...
export const SettingsModal: React.FC<SettingsModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 backdrop-blur-sm p-4">
      <div className="bg-[#111622] border border-slate-700 rounded-lg w-full max-w-md shadow-2xl overflow-hidden">
        {/* Header */}
        <div className="px-4 py-3 border-b border-slate-800 flex justify-between items-center">
          <div className="flex items-center gap-2">
            <Sliders className="w-4 h-4 text-cyan-400" />
            <h3 className="font-bold text-sm text-white">System & Agent Settings</h3>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-4 text-xs">
          ...
          <div className="pt-2 border-t border-slate-800 flex justify-end gap-2">
            ...
            <button
              type="submit"
              disabled={isSaving}
              className="px-4 py-1.5 rounded bg-blue-600 hover:bg-blue-500 text-white font-semibold text-xs shadow"
            >

...
export const TickerBanner: React.FC<TickerBannerProps> = ({
  ...
  return (
    <div className="bg-[#171f30] border-b border-slate-800 px-4 py-2 flex items-center gap-5 overflow-x-auto text-xs">
      ...
      {/* 30-Day Macro Context Strip */}
      {monthlyContext && (
        <>
          <div className="flex flex-col min-w-[200px]">
            <div className="flex justify-between items-center text-[10px] font-mono">
              <span className="text-slate-500 uppercase tracking-wider">30D Macro Range</span>
              <span
            ...
            <div className="flex justify-between text-[10px] font-mono">
              <span className="text-slate-400">Low: ${formatPrice(monthlyContext.monthly_low)}</span>
              <span className="text-cyan-400 font-bold">{rangePos.toFixed(0)}% channel</span>
              <span className="text-slate-400">High: ${formatPrice(monthlyContext.monthly_high)}</span>
            </div>
          </div>
